...
class TestConfigFlow:
    ...
    @pytest.mark.asyncio
    async def test_user_step_single_box_success(
        self, hass, mock_setup_entry, mock_msp_client, mock_aiohttp_session
    ):
        ...
        with patch.object(
            flow, "_authenticate_msp", new_callable=AsyncMock
        ), patch.object(
            flow, "_get_available_boxes", new_callable=AsyncMock
        ), patch.object(
            flow, "_test_rule_access", new_callable=AsyncMock
        ), patch.object(
    ...
    @pytest.mark.asyncio
    async def test_user_step_multiple_boxes(
        self, hass, mock_msp_client, mock_aiohttp_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_user_step_no_boxes(
        self, hass, mock_msp_client, mock_aiohttp_session
    ):
    ...
    async def test_user_step_auth_failed(self, hass, mock_aiohttp_session):
        ...
        with patch.object(
            flow, "_authenticate_msp", side_effect=InvalidAuth("Auth failed")
        ):
            result = await flow.async_step_user(
                {
                    CONF_MSP_URL: "test.firewalla.net",
    ...
    async def test_user_step_connection_failed(self, hass, mock_aiohttp_session):
        ...
        with patch.object(
            flow, "_authenticate_msp", side_effect=CannotConnect("Connection failed")
        ):
            result = await flow.async_step_user(
                {
                    CONF_MSP_URL: "test.firewalla.net",
    ...
    async def test_user_step_rule_access_failed(self, hass, mock_aiohttp_session):
        ...
        with patch.object(
            ...
            flow,
            "_test_rule_access",
            side_effect=RuleAccessFailed("Rule access failed"),
        ):
    ...
    @pytest.mark.asyncio
    async def test_authenticate_msp_success(
        self, hass, mock_msp_client, mock_aiohttp_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_available_boxes_success(
        self, hass, mock_msp_client, mock_aiohttp_session
    ):
